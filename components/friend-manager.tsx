    ...
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-md p-4 space-y-3">
        ...
        {results.length > 0 && (
          <div className="space-y-2">
            {results.map((u) => (
              <div key={u.id} className="flex items-center gap-3 rounded-lg border border-zinc-800 p-2">
                ...
                <span className="flex-1 text-sm font-medium text-zinc-100">{u.name ?? "Anonymous"}</span>
                <Badge className="bg-zinc-800 text-zinc-400 border-zinc-700">Lvl {u.level}</Badge>
                <Button
                  size="sm"
      ...
      {incoming.length > 0 && (
        <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-md p-4 space-y-3">
          ...
          {incoming.map((req) => (
            <div key={req.friendshipId} className="flex items-center gap-3 rounded-lg border border-zinc-800 p-2">
              <Avatar className="h-8 w-8">
              ...
              <button
                ...
                className="p-1.5 rounded-full bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20 hover:shadow-[0_0_8px_1px_rgba(16,185,129,0.5)] transition"
              >
                <Check size={16} />
              </button>
              <button
                onClick={() => respond(req.friendshipId, "DECLINED")}
                className="p-1.5 rounded-full bg-red-500/10 text-red-400 hover:bg-red-500/20 hover:shadow-[0_0_8px_1px_rgba(239,68,68,0.5)] transition"
              >
                <X size={16} />
              </button>
            </div>
      ...
      <div className="rounded-2xl border border-zinc-800 bg-zinc-900/50 backdrop-blur-md p-4 space-y-3">
        ...
        {friends.length === 0 && (
          <p className="text-sm text-zinc-600">No friends yet — search above to add some.</p>
        )}
        {friends.map((f) => (
          <button
            key={f.id}
            onClick={() => setSelectedFriendId(f.id)}
            className="flex items-center gap-3 rounded-lg border border-zinc-800 p-2 w-full text-left hover:border-zinc-700 hover:bg-zinc-900/40 transition-colors"
          >
            <Avatar className="h-8 w-8">
            ...
            <span className="flex-1 text-sm font-medium text-zinc-100">{f.name ?? "Anonymous"}</span>
            <Badge className="bg-zinc-800 text-zinc-400 border-zinc-700">Lvl {f.level}</Badge>
            <span className="font-mono text-xs text-purple-400 w-16 text-right">{f.xp} XP</span>
          </button>
        ))}
      </div>
